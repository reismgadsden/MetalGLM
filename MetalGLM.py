"""
This program will explore other regression models that are offered by the
statsmodels package.

author: Reis Gadsden 2022-09-23
modified: 2022-09-23
class: CS-5245 @ Appalchian State University
instructor: Dr. Mitchell Parry
github: https://github.com/reismgadsden/MetalGLS
"""
...
class MetalGLM:
    ...
    def __init__(self, csv=None, cid=None, scid=None):
        ...
        # we already got the popularity in MetalRegression.py
        # this information was exported to a csv to avoid query
        # spotify API for the same exact data
        if "Popularity" not in self._df.columns:
    ...
    def pred_pop_by_genre(self):
        ...
        res_lm = smf.ols(formula="popularity ~ genre -1", data=ols_df).fit()
        p_values = res_lm.summary2().tables[1]['P>|t|']

        res_gaus_log = sm.GLM(family=sm.families.Gaussian(link=sm.l))

        with open("pred_gen_w_pop.txt", 'w+') as file:
            file.write(str(res_lm.summary()) + "\n")
...
if __name__ == "__main__":
    ...
    # initialize stuff
    mr = MetalGLM(csv=wrangle_csv, cid=cid, scid=scid)